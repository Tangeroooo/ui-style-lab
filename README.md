...
UI Style Lab is an interactive, bilingual reference for composing complete interface systems. `Page Lab` applies every selection to the same full-page `Field Notes` site, while the separate `Component Lab` carries the same selection into 48 reusable, interactive component specimens and their important states.
...
| --- | ---: | --- |
| Base aesthetic | 46 | The governing visual language, including historical lineages, selected company systems, documented dark variants, MUI, and SAP accessibility themes |
...
| Palette | 49 | Includes Classic System, Arcade, Space Age, a four-color Enterprise Rail family, Pure White, and native Atlassian, Primer, SAP Horizon/Quartz/Belize, Material, MUI, Fluent, Carbon, shadcn/ui, Zag, Tamagui, and Nebular token families |
| Motion | 6 | Quiet, subtle, kinetic, productive, staged, and spring physics |

The lab exposes a different number of visually distinct combinations for each content mode: **118,372** in English, **371,944** in Korean-only, and **1,477,940** in Korean + English. The count is compatibility-aware rather than a global Cartesian product. Latin type is not counted when Korean Unified makes it visually irrelevant, and the role-based Kakao pair is excluded from that unified branch. Incompatible values remain visible but disabled with a reason.

The added options were deliberately curated from the broader research list:

- Historical base aesthetics: `Bauhaus`, `Art Deco`, `Scandinavian`, `Classic Desktop GUI`, `Pixel UI`, and `Space-age Retrofuturism`. The three retro branches remain separate: Classic Desktop GUI reconstructs the dense desktop metaphor and raised/sunken control grammar; Pixel UI uses a low-resolution grid, hard edges, and a deliberately limited palette; Space-age Retrofuturism draws on mid-century Googie geometry and technological optimism. Each has a page-wide geometry, hierarchy, component, chart, and navigation treatment rather than a palette-only skin. Sources: [Microsoft border-style guidance](https://learn.microsoft.com/en-us/previous-versions/windows/desktop/bb226804(v=vs.85)), [Whitney Museum on bitmapped pixels](https://whitney.org/essays/histories-of-the-digital-now), and [Smithsonian on Googie](https://www.smithsonianmag.com/history/googie-architecture-of-the-space-age-122837470/).
