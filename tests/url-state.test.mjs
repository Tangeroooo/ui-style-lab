...
test("each content mode reports only visually distinct combinations", () => {
  assert.equal(experienceCombinationCount("en", "mixed"), 62_804);
  assert.equal(experienceCombinationCount("ko", "only"), 142_232);
  assert.equal(experienceCombinationCount("ko", "mixed"), 635_840);
});
